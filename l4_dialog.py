...
class L4Dialog(tk.Toplevel):
    def __init__(self, master, emp_manager: EmployeeManagement, emp_id, emp_name):
        ...
        self.title(f"Zwolnienie L4 - {emp_name}")
    ...
    def create_widgets(self):
        ...
        tk.Label(header_frame, text="🏥 Zwolnienie L4", font=('Arial', 16, 'bold')).pack()
        tk.Label(header_frame, text=f"Pracownik: {self.emp_name}", font=('Arial', 12)).pack()
        
        # Ramka z formularzem
        form_frame = ttk.LabelFrame(self.scrollable_frame, text="Dane zwolnienia", padding="20")
        ...
        # Informacje o L4
        info_frame = ttk.LabelFrame(self.scrollable_frame, text="Podsumowanie L4", padding="15")
        info_frame.pack(fill='x', pady=(0, 20))
        
        self.days_label = tk.Label(info_frame, text="Liczba dni kalendarzowych: 0", 
                                 font=('Arial', 11, 'bold'))
        ...
        ttk.Button(btn_frame, text="🔄 Oblicz", command=self.calculate_l4, 
                  width=12).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="💾 Zapisz L4", command=self.save_l4, 
    ...
    def calculate_l4(self):
        try:
            ...
            self.days_label.config(text=f"Liczba dni kalendarzowych: {total_days}")
    ...
    def save_l4(self):
        try:
            ...
            self.emp_manager.db.execute_query("""
                INSERT INTO l4_records (employee_id, start_date, end_date, total_days)
                VALUES (?, ?, ?, ?)
            """, (self.emp_id, start.date(), end.date(), total_days))
            
            # Zmień status pracownika na "L4"
            self.emp_manager.update_employee_status(self.emp_id, "L4")
            
            messagebox.showinfo("Sukces", 
                              f"Zwolnienie L4 zarejestrowane!\n\n"
                              f"Okres: {total_days} dni\n"
            ...
            messagebox.showerror("Błąd", f"Nie udało się zapisać L4: {e}")
