...
class EmployeeDialog(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        self.stanowisko_combo = ttk.Combobox(
            main_frame,
            textvariable=self.stanowisko_var,
        ...
        self.wydzial_combo = ttk.Combobox(
            main_frame,
            textvariable=self.wydzial_var,
        ...
        self.zmiana_combo = ttk.Combobox(
            zmiana_frame,
            textvariable=self.zmiana_var,
        ...
        ttk.Checkbutton(
            main_frame,
            text="Nie nadpisuj statusu przy Urlop/L4",
            variable=self.protect_absence_status_var
        ).grid(row=6, column=1, columnspan=2, sticky='w', padx=(10, 0), pady=(0, 4))

        # Maszyna/Urządzenie
        ttk.Label(main_frame, text="Maszyna/Urządzenie:").grid(row=7, column=0, sticky="w", pady=5)
        self.maszyna_var = tk.StringVar()
        self.maszyna_combo = ttk.Combobox(
            main_frame,
            textvariable=self.maszyna_var,
        ...
        ttk.Button(button_frame, text="Zapisz", command=self.save_employee, style='Accent.TButton').pack(side="left", padx=5)
        ...
        try:
            ...
            if letter in cfg:
                ...
                if getattr(self, 'protect_absence_status_var', None) and self.protect_absence_status_var.get():
                    try:
                        has_vac = self.emp_manager.get_any_vacation(self.emp_id)
                        has_l4 = self.emp_manager.get_any_l4(self.emp_id)
    ...
    def create_vacation_l4_section(self, main_frame):
        ...
        # Pobierz aktywne i najbliższe przyszłe dane o urlopach i L4
        self.current_vacation = self.emp_manager.get_active_vacation(self.emp_id)
        self.any_vacation = self.emp_manager.get_any_vacation(self.emp_id)
        self.current_l4 = self.emp_manager.get_active_l4(self.emp_id)
        self.any_l4 = self.emp_manager.get_any_l4(self.emp_id)
        ...
        elif self.any_vacation:
            start_date = self.any_vacation[1]
            end_date = self.any_vacation[2]
            vacation_info = f"Najbliższy urlop: {start_date} - {end_date}"
        ...
        if self.current_l4:
            ...
            start_date = self.any_l4[1]
            end_date = self.any_l4[2]
            l4_info = f"Najbliższe L4: {start_date} - {end_date}"
        ...
        ttk.Button(
            ...
            state="normal" if (self.current_vacation or self.any_vacation) else "disabled",
            width=12
        ...
        ttk.Button(
            ...
            state="normal" if (self.current_l4 or self.any_l4) else "disabled",
            width=12
        ).pack(side="left", padx=5)
        ...
        ttk.Button(
            ...
            state="normal" if (self.current_vacation or self.any_vacation or self.current_l4 or self.any_l4) else "disabled",
            width=12
        ).pack(side="left", padx=5)

    def delete_vacation(self):
        """Aktywny lub najbliższy przyszły wpis – usunięcie po ID + odśwież UI."""
        if messagebox.askyesno("Potwierdzenie", "Czy na pewno chcesz usunąć urlop tego pracownika?"):
            try:
                any_next = self.emp_manager.get_any_vacation(self.emp_id)
                target_id = any_next[0] if any_next else None
                if target_id and self.emp_manager.delete_vacation_by_id(target_id):
                    # jeśli obecny status to absencja, przywróć "W Pracy"
                    try:
                        if self.employee_data and len(self.employee_data) > 6 and self.employee_data[6] in ("Urlop","L4"):
                            self.emp_manager.update_employee_status(self.emp_id, "W Pracy")
                    except Exception:
                        pass
                    emp_name = f"{self.employee_data[1]} {self.employee_data[2]}" if self.employee_data else str(self.emp_id)
                    self.emp_manager.log_history("Usunięcie urlopu", "Usunięto urlop dla pracownika " + emp_name, self.emp_id)
                    messagebox.showinfo("Sukces", "Usunięto wpis.")
                    if hasattr(self.master, 'refresh_employee_list'):
                        self.master.refresh_employee_list()
                    self.refresh_absence_section()
                else:
                    messagebox.showerror("Błąd", "Nie znaleziono wpisu do usunięcia.")
    ...
    def delete_l4(self):
        ...
        if messagebox.askyesno("Potwierdzenie", "Czy na pewno chcesz usunąć L4 tego pracownika?"):
            try:
                any_next = self.emp_manager.get_any_l4(self.emp_id)
                target_id = any_next[0] if any_next else None
                if target_id and self.emp_manager.delete_l4_by_id(target_id):
                    # jeśli obecny status to absencja, przywróć "W Pracy"
                    try:
                        if self.employee_data and len(self.employee_data) > 6 and self.employee_data[6] in ("Urlop","L4"):
                            self.emp_manager.update_employee_status(self.emp_id, "W Pracy")
                        ...
                        pass
                    emp_name = f"{self.employee_data[1]} {self.employee_data[2]}" if self.employee_data else str(self.emp_id)
                    self.emp_manager.log_history("Usunięcie L4", "Usunięto L4 dla pracownika " + emp_name, self.emp_id)
                    messagebox.showinfo("Sukces", "L4 zostało usunięte.")
                    if hasattr(self.master, 'refresh_employee_list'):
                        self.master.refresh_employee_list()
                    self.refresh_absence_section()
                else:
                    messagebox.showerror("Błąd", "Nie znaleziono wpisu L4 do usunięcia.")
            except Exception as e:
    ...
    def delete_all_absences(self):
        ...
        if messagebox.askyesno("Potwierdzenie", "Czy na pewno chcesz usunąć wszystkie nieobecności tego pracownika?"):
            try:
                self.emp_manager.db.execute_query("DELETE FROM vacations WHERE employee_id = ?", (self.emp_id,))
                self.emp_manager.db.execute_query("DELETE FROM l4_records WHERE employee_id = ?", (self.emp_id,))
                if self.employee_data and len(self.employee_data) > 6 and self.employee_data[6] in ["Urlop", "L4"]:
                    self.emp_manager.update_employee_status(self.emp_id, "W Pracy")
                emp_name = f"{self.employee_data[1]} {self.employee_data[2]}" if self.employee_data else str(self.emp_id)
    ...
    def save_employee(self):
        ...
        # Zapisz pracownika
        if self.employee_data:
            result = self.emp_manager.update_employee(self.employee_data[0], imie, nazwisko, stanowisko, wydzial, zmiana, status, maszyna)
        else:
            result = self.emp_manager.add_employee(imie, nazwisko, stanowisko, wydzial, zmiana, status, maszyna)
    ...
    def refresh_absence_section(self):
        """Odświeża sekcję nieobecności i stany przycisków."""
        try:
            self.current_vacation = self.emp_manager.get_active_vacation(self.emp_id)
            self.any_vacation = self.emp_manager.get_any_vacation(self.emp_id)
            self.current_l4 = self.emp_manager.get_active_l4(self.emp_id)
            self.any_l4 = self.emp_manager.get_any_l4(self.emp_id)
            # Znajdź istniejącą ramkę i przebuduj
            for child in self.winfo_children():
                if isinstance(child, ttk.LabelFrame) and child.cget('text') == 'Zarządzanie urlopami i L4':
                    container = child.master
                    child.destroy()
                    self.create_vacation_l4_section(container)
                    break
        except Exception:
            pass
