...
class EmployeeManagement:
    ...
    def add_employee(self, imie, nazwisko, stanowisko, wydzial, zmiana, status, maszyna):
        try:
            ...
            # automatyczne ustawienie statusu wg zmiany (dodawanie – bez ochrony; absencje skorygują status później)
            if zmiana == "D":
                status = "Wolne"
    ...
    def update_employee(self, emp_id, imie, nazwisko, stanowisko, wydzial, zmiana, status, maszyna):
        ...
        try:
            ...
            # Ochrona Urlop/L4 (teraz i przyszłość) przy auto-statusie
            if not self.has_any_absence_now_or_future(emp_id):
    ...
    def move_employee(self, emp_id, new_wydzial=None, new_zmiana=None, new_stanowisko=None):
        ...
        if new_zmiana and new_zmiana != old_data[1]:
            ...
            # Ochrona Urlop/L4 (teraz i przyszłość)
            if not self.has_any_absence_now_or_future(emp_id):
    ...
    def add_setting(self, table_name, data):
        if table_name == 'users':
            if not data.get('password') or not data.get('role'):
                return False
    ...
    def has_any_absence_now_or_future(self, emp_id):
        """True jeśli pracownik ma aktywny LUB przyszły Urlop/L4 (end_date >= dziś)."""
        try:
            vac = self.get_any_vacation(emp_id)
            l4 = self.get_any_l4(emp_id)
            return bool(vac or l4)
        except Exception:
            return False
    ...
    def apply_statuses_from_shifts(self, skip_absences=True):
        """Ustawia status 'Wolne' dla pracowników ze zmianą 00:00–00:00,
        a 'W Pracy' dla pozostałych (A/B/C/D). Gdy skip_absences=True
        nie nadpisuje statusu pracowników z aktywnym lub przyszłym Urlopem/L4 (end_date >= dziś)."""
        try:
                    ...
                    if skip_absences:
                        self.db.execute_query(
                            """                            UPDATE employees
                               ...
                               WHERE zmiana=?
                                 AND id NOT IN (
                                     SELECT employee_id FROM vacations WHERE date(end_date) >= date('now')
                                     UNION
                                     SELECT employee_id FROM l4_records WHERE date(end_date) >= date('now')
                                 )
                            """, (key,))
                    else:
