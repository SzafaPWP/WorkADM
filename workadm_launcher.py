# workadm_launcher.py  -- użyj tego jako launcher
import sys
import subprocess
...
def find_pythonw():
    # jeżeli używasz venv, pythonw.exe zwykle jest obok python.exe
    py = Path(sys.executable)
    pythonw = py.parent / "pythonw.exe"
    if pythonw.exists():
        return str(pythonw)
    # spróbuj znaleźć pythonw w PATH
...
def main():
    ...
    try:
        if pythonw:
            # użyj pythonw.exe (uruchamia GUI bez konsoli, okno powinno być widoczne)
            subprocess.Popen([pythonw, str(main_py)], cwd=str(prog_dir))
        else:
            # fallback: użyj cmd start, co zachowuje zachowanie jak dwuklik
            # "" to title (wymagane), use start /B jeśli chcesz bez konsoli (nie zawsze)
